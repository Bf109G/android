...
abstract class BottomNavigationTab extends FrameLayout {
    ...
    @CallSuper
    public void initialise(boolean setActiveColor, boolean isOriginIcon) {
        iconView.setSelected(false);
        if (isInActiveIconSet) {
            ...
            if (setActiveColor) {
                if (isOriginIcon) {
                    mCompactInActiveIcon = mCompactIcon.getConstantState().newDrawable();
                    mCompactInActiveIcon.mutate();
                    DrawableCompat.setTint(mCompactInActiveIcon, mInActiveColor);
                    StateListDrawable states = new StateListDrawable();
                    states.addState(new int[]{android.R.attr.state_selected},
                            mCompactIcon);
                    states.addState(new int[]{-android.R.attr.state_selected},
